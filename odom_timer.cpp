...
public:
    OdometryCalculator() : Node("odom"), x_(0.0), y_(0.0), theta_(0.0)
    {
        ...
        wheel_speed_subscriber_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            "microROS/two_wheel_speed", 10,
            std::bind(&OdometryCalculator::wheelSpeedCallback, this, std::placeholders::_1));
...
private:
    void wheelSpeedCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg)
    {
    ...
    double dt = 0.0;
    double prev_time_stamp = this->now().seconds();
};
